...
To install a specific version of the operator theoretically we could use something like `startingCSV: amq-broker-operator.v7.10.1-opr-1` in the `Subscription`, which doesn't make much sense at this point being only the latest version available.

We can check the latest version available in our catalog-index by running `oc get packagemanifest amq-broker-rhel8` and look for `currentCSV` for the channels. Checking all version available in the catalog-index is more challenging:
